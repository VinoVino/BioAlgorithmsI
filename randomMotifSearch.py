...
"""def genRandomKmer(strand,kmerLength): #ranomly generates kmers to form motif
    #strandKmers=generateKmers(strand,kmerLength)
    random.seed()
    randomNumber=randint(0,len(strandKmers)-1)
    randomKmer=strandKmers[randomNumber]

    #speed this step up. have all kmers for each strand stored in list, pass in that list for that strand to pick random
...
def randomGreedyMotif(Sequences,kmerLength):
    ...
    while i < 1000:
        ...
        if Scores(currentMotif) < Scores(bestMotifs):  # if scores is better
            print "score--", Scores(currentMotif)
            bestMotifs=currentMotif   # reset bestMotif
            motif = bestMotifs   # set motif

        else: # if score is worse, recreate motif
            motif=[]
